...
namespace ISL
{
    class TeamCleansheet
    {
        private IWebDriver driver;
        private object missing;

        public TeamCleansheet(IWebDriver driver)
        {
            this.driver = driver;
        }
        public IWebElement GetTeamCleanSheetsData()
        {
            ...
            foreach (var div in allDivs)
            {
                ...
                var teamCleanSheets = div.FindElement(By.CssSelector(".si-goals")).Text;

                Team.Add(new Team { TeamName = teamname, TeamCleanSheets = teamCleanSheets });
                TestContext.Out.WriteLine($"TeamName: { teamname }| TeamCleanSheets: { teamCleanSheets } ");
            }
